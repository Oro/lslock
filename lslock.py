...
@click.command()
@click.argument(
    'directory', type=click.Path(exists=True), default="/tmp/lslock-test")
@click.option(
    '--verbose',
    '-v',
    default=False,
...
def display_locked_files(directory, verbose):
    ...
    for lock in locks:
        click.echo('File {} is locked by PID {}'.format(
            click.format_filename(lock.get('filepath')), lock.get('pid')))
    if not locks:
...
@click.argument(
    'directory', type=click.Path(exists=True), default="/tmp/lslock-test")
@click.option(
    '--number', '-n', default=5, help='How many lock files are generated')
@click.option(
    '--verbose',
    '-v',
    default=False,
...
def create_locks(directory, verbose, number):
    """Creates some locked files in directory
    and checks the validity of lslock"""
    if verbose:
        logging.basicConfig(level=logging.DEBUG)
...
def locked_files_in_dir(directory):
    """Returns a tuple of dicts of all filepaths in directory
    ...
    with open("/proc/locks", "r") as locks:
        for line in locks:
            pid = line.split()[4]  # process locking the file
            inode = line.split()[
                5]  # has the format MAJOR-DEVICE:MINOR-DEVICE:INODE-NUMBER
            inode = inode.split(":")[2]  # interested only in inode number
            all_locks[int(inode)] = int(pid)

    # The intersection of all locks and the locks in the specified directory
    intersection = set(all_locks) & set(files_in_directory)

    logging.info("All current locks: {}".format(all_locks))
    # to get a dict with both pid and filepath,
    # we need to append the pid we got from /proc/locks and the associated
    # filepath from the file inside the directory
    for lock in intersection:
        locked_files.append({
            'pid': all_locks[lock],
            'filepath': files_in_directory[lock]
        })
    return locked_files


def inodes_in_dir(directory):
    """Descent into directory and return
    a dict of all inodes with their filepath"""
    inode_dict = {}
